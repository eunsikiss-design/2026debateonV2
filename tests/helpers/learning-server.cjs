...
function build(file,coach){
 ...
 const storage={getUser:uid=>Object.values(users).find(u=>u.uid===uid),getUsers:()=>[users.student],getStudentPracticeSessions:()=>[],getStudentBadges:()=>[],getClassSettings:()=>({requiredBadgeCount:1}),savePracticeSession:input=>(sessions.push(input),{...input,sessionId:'fixture-'+sessions.length}),awardBadge:()=>null,getClassStudentsStatus:()=>[],getClassAnalytics:()=>({}),getTeacherObservations:()=>[]};
