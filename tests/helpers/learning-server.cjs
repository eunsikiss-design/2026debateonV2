...
function build(file,coach){
 const learning=new LearningService(file),users={student:{uid:'fixture-student',role:'student',name:'테스트 학생',schoolId:'fixture-school',grade:1,classId:1,studentNumber:'10101',email:'student@example.test',onboardingComplete:true},teacher:{uid:'fixture-teacher',role:'teacher',name:'테스트 교사',schoolId:'fixture-school',grade:1,classId:1,onboardingComplete:true}},sessions=[];
 const storage={getUser:uid=>Object.values(users).find(u=>u.uid===uid),getUsers:()=>[users.student],getStudentPracticeSessions:uid=>sessions.filter(s=>s.userId===uid).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)),getStudentBadges:()=>[],getClassSettings:()=>({requiredBadgeCount:1}),savePracticeSession:input=>(sessions.push(input),{...input,sessionId:'fixture-'+sessions.length}),awardBadge:()=>null,getClassStudentsStatus:()=>[],getClassAnalytics:()=>({}),getTeacherObservations:()=>[]};
 const auth={isConfigured:()=>true,serverConfigured:()=>true,clientConfigured:()=>true,authenticate:async req=>users[/test_role=(student|teacher)/.exec(req.headers.cookie||'')?.[1]],safeProfile:u=>u,sameClass:()=>true};
 const services={storageService:storage,learningService:learning,geminiService:coach||{evaluateBasicPractice:async()=>{throw Error('No coaching fixture configured');}},firebaseAuth:auth,socialAuth:{},studentRoster:{students:[],registrationStatus:()=>[]},knowledgeService:{cards:[],getEvidenceCards:()=>[],getStats:()=>({}),getGlossary:()=>[]},sheetSyncQueue:{enqueue(){},getQueueStatus:()=>({queueLength:0})}};
