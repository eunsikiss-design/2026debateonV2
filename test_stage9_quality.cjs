...
test('HTTP review writes use authenticated identity and reject student/cross-origin requests',async()=>{
 ...
 vm.runInNewContext(fs.readFileSync(path.join(__dirname,'server.js'),'utf8'),{require:n=>n==='http'?{createServer:f=>(handler=f,{listen(){}})}:n==='fs'?fs:n==='path'?path:n.includes('firebaseAuth')?auth:n.includes('knowledgeService')?knowledge:n.includes('storageService')?{getUser:()=>null}:n.includes('studentRoster')?{students:[],registrationStatus:()=>[]}:{},__dirname,process:{env:{},loadEnvFile(){}},console:{log(){},warn(){}},URLSearchParams,Buffer});
