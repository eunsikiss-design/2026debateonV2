...
class LearningService {
 constructor(file=process.env.TEACHING_CONFIG_PATH||path.join(process.env.DATA_STORE_PATH?path.dirname(path.resolve(process.env.DATA_STORE_PATH)):path.join(__dirname,'../data'),'teaching-config.json')){this.file=file;}
 ...
 mutate(user,method,id,input={}){
  ...
  db.schools[scope]={revision:current.revision+1,keywords:list,updatedAt:new Date().toISOString(),updatedBy:user.uid};
 ...
 topic(id,user,config=this.config(user)){const original=topics.find(t=>t.topicId===id);if(!original)fail('주제를 찾을 수 없습니다.',404,'TOPIC_NOT_FOUND');const words=config.keywords.filter(w=>w.topicIds.includes(id));return {...original,keyConcepts:words.map(w=>w.term),conceptDefinitions:words,keywordRevision:config.revision};}
 lesson(id,user){const topic=this.topic(id,user),m=this.material(id);return {topic,scenario:m?.scenario||{title:'만약에',story:topic.background,complication:'이 선택으로 도움을 받는 사람과 어려워지는 사람이 다를 수 있어요.',invitation:'나라면 어떤 선택을 할까요? 내 생각과 이유를 말해 보세요.'},speechScenario:m?.speechScenario||null,feedbackPoints:m?.taskChecks||[],basicRubric:BASIC_RUBRIC,activities:ACTIVITIES,hasMaterials:!!m};}
