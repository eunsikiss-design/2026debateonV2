...
class EvidenceReviewStore {
 ...
 decorate(card,role){const r=this.state(card);return {...card,reviewStatus:r.status,reviewRevision:r.revision,verificationStatus:r.status==='approved'?'verified':'source_extracted',...(role==='teacher'?{reviewNote:r.note||'',reviewedAt:r.reviewedAt||null}: {})};}
 save(card,input,user){
  ...
  const record={cardId:card.cardId,fingerprint:fingerprint(card),status:input.status,note:input.note.trim(),reviewerUid:user.uid,reviewedAt:new Date().toISOString(),revision:prior.revision+1,originalChecked:input.originalChecked===true};
