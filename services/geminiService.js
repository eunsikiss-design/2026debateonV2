...
`.trim();
  ...
  async planAdvancedWriting({lesson,writingPlan}) {
    ...
    if(this.apiKey&&this.coachModel){
      try{
        const result=await this._callGeminiAPIWithPrompt({modelName:this.coachModel,responseSchema:writingQuestions.schema,temperature:0.3,
        ...
        return {source:'gemini-api',writingPlan:plan,paragraphs:writingQuestions.validate(result,lesson,plan)};
