...
class LearningDraftStore{
 ...
 save(user,topicId,input){
  const {mode,content,revision}=input;if(!['basic','advanced'].includes(mode)||!content||typeof content!=='object')error('초안 형식을 확인하세요.');
  ...
  else{if(!Array.isArray(content.paragraphs)||content.paragraphs.length>20||content.paragraphs.some(p=>typeof p!=='string'||p.length>10000)||content.paragraphs.join('').length>40000)error('논술 초안 분량을 확인하세요.');value={paragraphs:content.paragraphs,writingPlan:require('../assets/writing-plan').normalize(content.writingPlan)};}
