(() => {
  ...
  function outline(){
    ...
    [['1. 나의 주장',claim],['2. 이유와 상황',reason],['3. 다른 의견 또는 조건',condition||'말하면서 떠올려도 좋아요.']].forEach(([heading,value])=>{const part=document.createElement('p');const strong=document.createElement('strong');strong.textContent=heading+' · ';part.append(strong,document.createTextNode(value));node.append(part);});
    node.hidden=false;outlineReady=true;setState(state,'개요를 보며 자신의 말로 스피치해 보세요.');
  }
  function reset(){
    clearInterval(timer);stopRecognition();recognition=null;activeSeconds=0;startedAt=0;finalTranscript='';interimTranscript='';$('speech-feedback').hidden=true;setState('READY','처음부터 다시 시작할 수 있습니다.');renderTimer();renderTranscript();
  }
  ...
  window.addEventListener('curriculum-topic-change',()=>{topicId=window.TopicCatalog.current?.topicId||null;reset();outlineReady=false;$('speech-outline').hidden=true;$('speech-claim').value='';$('speech-reason').value='';$('speech-condition').value='';$('topic-question').textContent=window.TopicCatalog.current?.question||'주제를 선택해 주세요.';setState('READY');});
  window.TopicCatalog.ready.then(()=>{topicId=window.TopicCatalog.current?.topicId||null;$('topic-question').textContent=window.TopicCatalog.current?.question||'주제를 선택해 주세요.';});
