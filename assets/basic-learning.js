(() => {
  ...
  let attempt=1,evaluation=null,evaluatedText='',feedbackStartingText='',busy=false,needsRewrite=false;
  ...
  function saving(){ $('final-submit-badge-btn').disabled=busy||!evaluation||attempt<2||needsRewrite||fingerprint()===feedbackStartingText; }
  function sync(){if(busy)return;attempt=1;evaluation=null;evaluatedText='';feedbackStartingText='';needsRewrite=false;$('attempt-badge').textContent='1회차 생각 쓰기';$('topic-question').textContent=window.TopicCatalog.current?.question||'주제를 선택해 주세요.';$('coach-feedback').hidden=true;$('request-eval-btn').disabled=!window.TopicCatalog.current;status.textContent='';selectedClaim();scaffold();saving();}
  ...
  document.addEventListener('change',event=>{if(event.target.matches('[name=stance]')){selectedClaim();if(needsRewrite&&fingerprint()!==feedbackStartingText)needsRewrite=false;saving();saveDraft();}});
  ...
  $('request-eval-btn').addEventListener('click',async()=>{
    ...
    try{const r=await fetch('/api/practice/evaluate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({topicId:window.TopicCatalog.current.topicId,stance:stance(),claim:claim.value,reason:reason.value,rebuttal:rebuttal.value,attemptCount:attempt,currentScaffoldLevel:evaluation?.scaffold?.recommendedLevel||1})});const data=await r.json();if(!r.ok||!data.evaluation)throw Error(data.error||'피드백을 받지 못했습니다.');evaluation=data.evaluation;evaluatedText=fingerprint();if(attempt===1)feedbackStartingText=evaluatedText;render(evaluation);status.textContent=attempt<2?'질문을 읽고 이유를 고쳐 보세요.':'피드백을 확인한 뒤 다시 쓴 글을 기록할 수 있습니다.';}
    catch(e){evaluation=null;status.textContent=e.message+' 입력한 글은 그대로 있습니다. 잠시 후 다시 요청해 주세요.';}
    finally{lock(false);}
  });
  $('retry-rewrite-btn').addEventListener('click',()=>{if(attempt<2)attempt=2;needsRewrite=fingerprint()===feedbackStartingText;$('attempt-badge').textContent=attempt+'회차 다시 쓰기';status.textContent='글을 고친 뒤 바로 기록할 수 있습니다. 원하면 소크라AI의 피드백을 다시 받을 수도 있습니다.';saving();reason.focus();});
  $('save-basic-draft-btn').addEventListener('click',async()=>{if(!draftSession)return;saveDraft();try{const version=await draftSession.snapshot();status.textContent=version+'차 저장본을 남겼습니다. 이전 저장본은 내 활동 기록에서 볼 수 있습니다.';window.dispatchEvent(new Event('activity-record-saved'));}catch(e){status.textContent=e.message;}});
  $('final-submit-badge-btn').addEventListener('click',async()=>{if($('final-submit-badge-btn').disabled)return;lock(true);try{const r=await fetch('/api/practice/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({topicId:window.TopicCatalog.current.topicId,stance:stance(),claim:claim.value,reason:reason.value,rebuttal:rebuttal.value,scaffoldLevel:evaluation.scaffold.recommendedLevel,evaluation,attemptCount:attempt})});const d=await r.json();if(!r.ok||!d.success)throw Error(d.error||'저장에 실패했습니다.');status.textContent=d.badgeAwarded?'다시 쓴 글을 기록하고 연습 뱃지를 받았습니다.':'다시 쓴 글을 기록했습니다.';evaluation=null;window.dispatchEvent(new Event('activity-record-saved'));}catch(e){status.textContent=e.message+' 글은 화면에 유지됩니다.';}finally{lock(false);}});
})();
