(() => {
  ...
  $('save-basic-draft-btn').addEventListener('click',async()=>{if(!draftSession)return;saveDraft();try{await draftSession.flush();status.textContent='현재 글을 이 주제의 초안으로 저장했습니다.';window.dispatchEvent(new Event('activity-record-saved'));}catch(e){status.textContent=e.message;}});
