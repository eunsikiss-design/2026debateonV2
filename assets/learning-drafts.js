(() => {
 ...
 function writingSource(data,mode){
  ...
  const draft=data.drafts?.[mode];for(const version of draft?.versions||[])if(hasText(version.content))candidates.push({...version,kind:'snapshot'});if(!candidates.length&&draft&&hasText(draft.content))candidates.push({...draft,kind:'draft'});
 ...
 async function open(topic,mode,status){
  ...
  const session={content:stored?.content||data.previous.filter(p=>p.mode===mode).at(-1)?.content||null,
   ...
   close(){clearTimeout(timer);session.flush().then(()=>sessions.delete(session)).catch(()=>{});}
