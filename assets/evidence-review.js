 ...
 async function load(){const id=++requestId;$('review-status').textContent='자료를 불러오는 중입니다.';
  try{const data=await api('/api/teacher/evidence?'+new URLSearchParams({status:$('review-state').value,sourceId:$('review-source').value,offset,limit:10}));if(id!==requestId)return;total=data.total;$('review-cards').replaceChildren();
   ...
   for(const card of data.cards){const panel=el('article','','cyber-panel');const stateLabel=el('p',names[card.reviewStatus],'evidence-note');panel.append(el('h2',card.source),stateLabel);
    ...
    const compare=el('div');compare.style.cssText='display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr));gap:16px';compare.append(el('p',card.extractedDetail||card.detail,'evidence-excerpt'));
    ...
    const form=el('form'),correctionLabel=el('label','교정 발췌문'),correction=el('textarea');correction.value=card.correctedDetail||card.detail;correction.minLength=12;correction.maxLength=12000;correction.id='correction-'+card.cardId;correctionLabel.htmlFor=correction.id;
    const help=el('p','OCR 오류나 문장 잘림을 원문에 맞게 고치세요. 승인된 교정본이 학생 검색과 AI 근거에 사용됩니다.','evidence-note');
    const label=el('label','검수 메모'),note=el('textarea');note.value=card.reviewNote||'';note.maxLength=2000;note.id='note-'+card.cardId;label.htmlFor=note.id;
    ...
    for(const [status,title] of [['approved','교정 후 승인'],['rejected','보류'],['pending','검수 전으로 되돌리기']]){const button=el('button',title,'evidence-button');button.type='button';button.addEventListener('click',async()=>{
      if(status==='approved'&&!check.checked){message.textContent='원문 대조 확인란을 체크해 주세요.';return;}
      if(!correction.value.trim()||correction.value.trim().length<12){message.textContent='교정 발췌문을 12자 이상 입력해 주세요.';return;}
      ...
      try{const result=await api('/api/teacher/evidence/review',{cardId:card.cardId,status,correctedDetail:correction.value,note:note.value,originalChecked:check.checked,revision:card.reviewRevision});card.reviewRevision=result.card.reviewRevision;card.reviewStatus=result.card.reviewStatus;card.correctedDetail=result.card.correctedDetail;correction.value=result.card.correctedDetail;stateLabel.textContent=names[card.reviewStatus];message.textContent=names[card.reviewStatus]+' 저장 완료 · 목록을 불러오면 필터에 반영됩니다.';}catch(error){message.textContent=error.message;}finally{controls.forEach(b=>b.disabled=false);}
    });actions.append(button);}form.append(correctionLabel,correction,help,label,note,checkLabel,actions,message);form.addEventListener('submit',event=>event.preventDefault());panel.append(form);$('review-cards').append(panel);
   }
