(() => {
  ...
  function view(name){if(!connected)return;currentPanel=name;$$('[data-view]').forEach(p=>p.hidden=p.dataset.view!==name);$$('#teacher-nav button').forEach(b=>b.setAttribute('aria-current',b.dataset.panel===name?'page':'false'));$('#teacher-page-title').textContent=pageTitles[name]||pageTitles.overview;document.title=`DebateOn · ${pageTitles[name]||pageTitles.overview}`;$('#teacher-main').focus({preventScroll:true});}
  ...
  async function load(){connected=false;state.className='connection-state';state.innerHTML='<div><strong>CONNECTING</strong><span>교사 권한과 담당 학급을 확인하고 있습니다.</span></div>';
    try{const me=await api('/api/auth/me');if(me.user.role!=='teacher')throw Object.assign(new Error('교사 계정으로 로그인해야 합니다.'),{status:403});teacher=me.user;$('#teacher-identity').textContent=`${teacher.name||'교사'} · TEACHER`;
      ...
      state.innerHTML='<div><strong>CONNECTED</strong><span>검증된 교사 세션과 담당 학급 범위로 연결되었습니다.</span></div>';connected=true;view(currentPanel);
