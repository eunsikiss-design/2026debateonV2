(() => {
 ...
 window.TeacherLessonEditor=(data,onSaved)=>{
  ...
  for(const [key,label,max]of [['question','토론 논제 (E열)',700],['essayPrompt','논술 주제',700],['textbookRef','교과서·쪽수',300],['debateGoal','토론 목표',1200],['essayGoal','논술 목표',1200],['setting',p.preserveEpisode?'생각을 여는 사례 · 에피소드 원문 (H열)':'생각을 여는 사례 · 공통 배경 (합니다·입니다체)',2500],['issue',p.preserveEpisode?'필수 쟁점 (I열)':'핵심 쟁점',700],['advancedComplication','심화 논술 · 추가 조건',2500]])fields[key]=field(key,label,p[key],max);
