(() => {
 ...
 window.TeacherLessonEditor=(data,onSaved)=>{
  ...
  form.append(n('h3','단원별 상황·목표 편집'),n('p',data.sourceNote,'source-note'),n('p','저장하면 학생이 주제를 다시 열 때 화면에 반영되고, 다음 AI 요청부터 최신 자료를 사용합니다. 인물의 주장에 대한 학생 자신의 판단을 묻습니다.'));
  ...
  for(const [key,label,max]of [['question','토론 논제',700],['essayPrompt','논술 주제',700],['textbookRef','교과서·쪽수',300],['debateGoal','토론 목표',1200],['essayGoal','논술 목표',1200],['setting','만약에 · 공통 배경 (합니다·입니다체)',2500],['issue','핵심 쟁점',700],['advancedComplication','심화 논술 · 추가 조건',2500]])fields[key]=field(key,label,p[key],max);
  ...
  form.append(n('p','발문은 “위 인물의 입장에서 제시한 ‘주장’에 동의합니까, 동의하지 않습니까?”로 표시됩니다. 학생에게 그 인물의 역할을 맡기지 않습니다.'));
  ...
  form.addEventListener('submit',async e=>{e.preventDefault();const plan={...p};Object.entries(fields).forEach(([key,input])=>plan[key]=input.value.trim());plan.characters=characters.map(c=>({name:c.name.value.trim(),context:c.context.value.trim(),position:c.position.value.trim()}));
