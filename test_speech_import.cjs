...
test('speech imports the newest nonempty own writing and falls back past empty drafts',()=>{
 ...
 assert.equal(writingSource(data,'basic').content.reason,'옛 기초 제출');assert.equal(writingSource(data,'advanced').content.paragraphs[0],'최신 논술 제출');
 data.drafts.basic.versions=[{version:1,updatedAt:'2026-09-26T12:00:00Z',content:{claim:'주장',reason:'마지막에 저장한 글'}}];
 assert.equal(writingSource(data,'basic').content.reason,'마지막에 저장한 글');
 data.drafts.basic.versions=[];
