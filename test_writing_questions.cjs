...
test('AI planning receives the actual character and plan, and rejects invented context or terms',async t=>{
 ...
 const lesson=learning.lesson('episode2026_1_02',user),plan=recommend('chars',300);
 ...
 assert.equal(fallback.source,'lesson-guide');assert.equal(fallback.paragraphs.length,2);assert.ok(lesson.scenario.story.includes(fallback.paragraphs[0].contextQuote));
});
test('planning endpoint authenticates, checks origin and uses canonical lesson data',async t=>{
 ...
 assert.equal((await call('',server.origin,{topicId:'episode2026_1_02'})).status,401);
 assert.equal((await call('test_role=student','https://outside.invalid',{topicId:'episode2026_1_02'})).status,403);
 const r=await call('test_role=student',server.origin,{topicId:'episode2026_1_02',lesson:{scenario:{story:'forged'}},writingPlan:recommend('chars',300)});
 assert.equal(r.status,200);assert.equal(r.headers.get('cache-control'),'no-store');assert.match(received.lesson.scenario.story,/정태훈/);
