...
vm.runInNewContext(fs.readFileSync(path.join(__dirname,'server.js'),'utf8'),{
  require:name=>name==='http'?{createServer:fn=>{handler=fn;return {listen(){}};}}:name==='fs'?fs:name==='path'?path:name.includes('knowledgeService')?knowledge:name.includes('storageService')?{getUser:()=>null}:name.includes('studentRoster')?{students:[],registrationStatus:()=>[]}:name.includes('firebaseAuth')?{
